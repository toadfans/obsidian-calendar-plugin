...
function getNoteTags(note: TFile | null): { tags: string[]; icons: string[] } {
  if (!note) {
    return { tags: [], icons: [] };
  }
  ...
  if (frontmatter) {
    ...
    icons.push(
      ...(frontmatter.icon ? [...frontmatter.icon].filter((a) => !!a) : [])
    );
  }

  // strip the '#' at the beginning
  return { tags: tags.map((tag) => tag.substring(1)), icons };
}

function getFormattedTagAttributes(note: TFile | null): Record<string, string> {
  const attrs: Record<string, string> = {};
  const { tags, icons } = getNoteTags(note);
